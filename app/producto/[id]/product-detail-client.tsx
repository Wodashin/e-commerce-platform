...
export default function ProductDetailClient({ product }: ProductDetailClientProps) {
  // --- SAFETY CHECKS (This fixes the 'reading 0' error) ---
  
  // 1. Safe access to images. If null, use a placeholder.
  const images = (product && Array.isArray(product.images) && product.images.length > 0) 
    ? product.images 
    : ["/placeholder.svg"]

  // 2. Safe access to sizes. If null, use an empty array.
  const sizes = (product && Array.isArray(product.sizes)) ? product.sizes : []
  
  // 3. Set initial size safely. If sizes exist, pick the first one; otherwise null.
  ...
  const handleWhatsAppContact = () => {
    ...
    const priceInfo = ` - Precio: $${formatPrice(finalPrice)}`
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-4">
          <div className="relative aspect-square bg-muted rounded-lg overflow-hidden border">
            ...
            <div className="absolute top-4 left-4 flex flex-col gap-2">
              {/* Optional: Check for 'new' status safely */}
        ...
        <div className="space-y-6">
          <div>
            ...
            <h1 className="text-3xl font-bold mb-2">{product?.name || "Nombre no disponible"}</h1>
            {product?.seller && (
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            <div className="flex gap-3">
              <Button
                ...
                // Disable if out of stock (safe check)
